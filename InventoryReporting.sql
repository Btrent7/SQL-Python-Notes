...
SIX_MO_INVOICE AS (
    SELECT I.ORDERWAREHOUSE AS PLT, ITEM, SUM(QTYSHIP) AS QTYSOLD, orderType
    FROM InvoiceDetailHistory I
    INNER JOIN DimDate DT ON DT.CALENDARDATE = I.invoiceDate
    WHERE calendardate >= DATEADD(MONTH, -7, GETDATE())
      AND calendardate < DATEADD(MONTH, -1, GETDATE())
    GROUP BY I.ORDERWAREHOUSE, ITEM, orderType
),

-- 3-month Moving Qty Sold 
THREE_MO_INVOICE AS (
    SELECT I.ORDERWAREHOUSE AS PLT, ITEM, (SUM(QTYSHIP)*2) AS QTYSOLD, orderType
    FROM InvoiceDetailHistory I
    INNER JOIN DimDate DT ON DT.CALENDARDATE = I.invoiceDate
...
TOTAL_INVOICES AS (
	SELECT S.PLT, S.ITEM, S.orderType, (T.QTYSOLD + S.QTYSOLD)/2 AS QTYSOLD
	FROM SIX_MO_INVOICE S
	JOIN THREE_MO_INVOICE T ON S.ITEM = T.ITEM AND S.PLT = T.PLT AND S.orderType = T.orderType
),
...
WEIGHTED_AVG AS (
    SELECT 
		...
		SUM(OHQTY - (OUTSTKQTY + COPSALCQTY)) AS ITMSTS, S.orderType,
	...
	GROUP BY D.PLT, D.ITMID, C.ITMDESC, C.PRDGRP, C.VNDID, C.ITMST,
		D.SYSAVGCST, D.OHQTY, D.POONORDQTY, D.OUTSTKQTY, S.QTYSOLD, S.orderType
),
...
SELECT 
	...
	D.ITMSTS, D.orderType, D.QTY_SOLD, 
