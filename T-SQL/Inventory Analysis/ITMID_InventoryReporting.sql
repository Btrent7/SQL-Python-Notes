-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
----- AGGREGATING INVENTORY BY ITMID -----
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
SIX_MO_INVOICE AS (
    SELECT ITEM, SUM(QTYSHIP) AS QTYSOLD
...
THREE_MO_INVOICE AS (
    SELECT ITEM, (SUM(QTYSHIP)*2) AS QTYSOLD
    FROM InvoiceDetailHistory I
    INNER JOIN DimDate DT ON DT.CALENDARDATE = I.invoiceDate
...
TOTAL_INVOICES AS (
	SELECT S.ITEM, (T.QTYSOLD + S.QTYSOLD)/2 AS QTYSOLD
	FROM SIX_MO_INVOICE S
	JOIN THREE_MO_INVOICE T ON S.ITEM = T.ITEM
),
...
WEIGHTED_AVG AS (
    SELECT 
        C.ITMID, C.ITMDESC, C.PRDGRP, C.VNDID,
		AVG(D.SYSAVGCST) AS AVG_COST, C.ITMST,
	...
	GROUP BY C.ITMID, C.ITMDESC, C.PRDGRP, C.VNDID, C.ITMST,
		C.SYSAVGCST, S.QTYSOLD
),
...
AVG_LEADTIME AS(
  SELECT D.ITMID, CAST(AVG(SYS_LT) AS DECIMAL (10,2)) AS SYS_LT,
    CASE 
...
LT_FINAL AS(
SELECT D.ITMID, D.AVG_LT, ((D.AVG_LT*5)/7) as AVG_LT_BD, ISNULL(D.SYS_LT, 0) AS SYS_LT
FROM AVG_LEADTIME D
),
...
FROM WEIGHTED_AVG D
LEFT JOIN LT_FINAL L ON L.ITMID = D.ITMID
